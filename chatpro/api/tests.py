...
class TembaHandlerTest(ChatProTest):
    ...
    def test_new_contact(self, mock_get_contact, mock_get_group):
        ...
        # GET is not allowed
        response = self.url_get('unicef', '%s?%s' % (url, 'contact=000-007&group=000-001'))
        self.assertEqual(response.status_code, 405)

        # bad request if you forget a parameter
        response = self.url_post('unicef', '%s?%s' % (url, 'group=000-001'))
        ...
        response = self.url_post('unicef', '%s?%s' % (url, 'contact=001-008&group=001-007'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_new_message(self, mock_get_contact, mock_get_group):
        ...
        # bad request if you forget a parameter
        response = self.url_post('unicef', '%s?%s' % (url, 'text=Hello%20World&group=000-001'))
        self.assertEqual(response.status_code, 400)
        ...
        response = self.url_post('unicef', '%s?%s' % (url, 'contact=000-001&text=Hello%20Again&group=001-007'))
        self.assertEqual(response.status_code, 200)
        new_room = Room.objects.get(group_uuid='001-007', name="New group")
